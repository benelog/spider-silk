...
- `body()`/`bodyJson()` hold at most 1MB and a `bodyNdjson` line at most 1MB, counted in bytes as they arrive; beyond that the request answers 413, and every later read of that body answers 413 too. Raise it with `app.bodyLimits(BodyLimits.defaults().maxBytes(8 * 1024 * 1024).maxNdjsonLineBytes(64 * 1024))`, or lift both with `BodyLimits.unlimited()`. `bodyStream()`/`bodyReader()` are never limited, so a large upload goes through them; form fields and multipart are the container's limits.
...
- A large upload never becomes a `byte[]`: `req.file(name).writeTo(path)` writes it to disk and `inputStream()` hands it to a parser. An optional upload is `req.fileOrNull(name)` (null when absent, including a file input left empty), and a field carrying several files is `req.files(name)`. An upload the container refuses is 413 over the multipart limit and 400 when it will not parse, through all three reads, never a null.
...
- `requestLogger((req, completion) -> ...)` reports `RequestCompletion`: `statusCode()` is the servlet status, `took()` a Duration, `response()` the response definition, `writeFailure()`/`writeFailed()` record decoration or writing failure independently of status, and `thrown()`/`threw()` report what the handler threw (null for an `HttpException`). Record failures on a span or in an error list from there, never from a catch-all `exception(...)` handler.
...
- Register everything before the app is served. Registration closes while an `AppServlet` serves the app — `app.start`, `new JettyServer(app).start()`, or an external container — and `stop()` reopens it. `app.cors/gzip/securityHeaders/staticFiles/bodyLimits(value)` copy the value, so changing it afterwards does nothing. An external container maps `AppServlet` with load-on-startup (`holder.setInitOrder(0)`, `<load-on-startup>0</load-on-startup>`).
...
| A 413 on a JSON body or an NDJSON import | `body()`/`bodyJson()` default to 1MB, and so does one `bodyNdjson` line. Raise it with `app.bodyLimits(BodyLimits.defaults().maxBytes(...))`, or read a body of any size through `bodyStream()` |
...
| A streamed response answers 200 and then fails | Once the headers are committed the status cannot change, so the transfer is aborted and the client sees an incomplete response. `RequestCompletion.writeFailure()` still reports the write failure. Whatever can fail in a way the client should hear about belongs before the response is returned |
