...
* Introspection: `routes()` reports every registered route as `Route(method, path)`, in registration order; `guards()` reports the filters and error handlers beside them as `Guard.Before(path)`, `Guard.After(path)`, `Guard.Error(status)`
