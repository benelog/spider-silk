...
There is no `WebContext`, so the expressions a Spring MVC page reaches for — `#request`, `#session`, `@bean` — are not there.
A model entry is: whatever the handler puts in the `Map` is what the page can read.
