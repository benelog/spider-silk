...
Production precompiles every template at build time with the https://jte.gg/pre-compiling/[jte Gradle plugin] and hands the engine the result:
...
xref:deployment.adoc[Deployment] has the Gradle side, the example project wires both modes behind a `--dev` flag, and a xref:native-image.adoc[native image] has no compiler at runtime at all, so there this mode is the only one.
