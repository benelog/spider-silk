 ...
 * The row mapping is written out too: each mapper is a lambda reading columns
 * by name, so no reflection touches the domain records — which is also what
 * lets the whole example compile to a native image without a reflection config.
 * SimpleJdbcInsert reads the table metadata from the database (not the record)
 * and hands back the generated key without a KeyHolder.
 */
public class CardRepository {
    ...
    private static final RowMapper<Card> MAPPER = (rs, rowNum) -> new Card(
    ...
    public Card insert(Card card) {
        Long id = insert.executeAndReturnKey(new MapSqlParameterSource()
                .addValue("deck_id", card.deckId())
                .addValue("text", card.text())
                .addValue("meaning", card.meaning())
                .addValue("created_at", card.createdAt())).longValue();
