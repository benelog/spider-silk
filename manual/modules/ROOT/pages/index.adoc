...
Every route, filter, and setting is registered before `start`.
Adding one while the application is served throws `IllegalStateException`, and `stop()` opens registration again.
...
`port()` reports the bound port, so a test that calls `start(0)` reads back the port the OS picked.
